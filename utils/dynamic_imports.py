...
def get_dboconfig_safe(function_name: str,user) -> Optional[str]:
    """
    ...
    if get_dboconfig_func:
        try:
            return get_dboconfig_func(function_name,user)
        except Exception as e:
            logger.warning(f"Error getting DB config: {e}")
    
    return None

def load_db_prompts(func_params: dict, db_deps: Dict[str, Any],user) -> Tuple[str, str, str, int, int]:
    """
    ...
    return db_query_prompt_loader(func_params,user)
