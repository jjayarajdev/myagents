...
def lazy_import_db_dependencies() -> Dict[str, Any]:
    ...
    get_dboconfig_func = get_function_from_module("db_agent.utils.helper_db_functions", "get_dboconfig")
    if get_dboconfig_func:
        db_deps["get_dboconfig"] = get_dboconfig_func
    
    # Try to import Queries
    queries_module = dynamic_import("db_agent.utils.query_repository")
    ...
    db_query_prompt_loader_func = get_function_from_module(
        "db_agent.func_executable.db_query_prompt_loader", "db_query_prompt_loader"
    )
    if db_query_prompt_loader_func:
        db_deps["db_query_prompt_loader"] = db_query_prompt_loader_func
    
    # Try to import db_query_exec
    db_query_exec_func = get_function_from_module("db_agent.utils.db_agent_utils", "db_query_exec")
